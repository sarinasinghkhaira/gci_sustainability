...
soc_eq_lolipop <- social_equity_rankings %>%
...
soc_eq_hist %>%
  filter(country_code %in% top_country_codes) %>%
  ggplot() +
  aes(x = year, y = ggi_index, group = country_code, colour = country_code == "GBR") +
  geom_line() +
...
At the current rate of increase in the uk, assuming its linear, what year will we reach 1?
