...
"It is possible for an economy to be growing, inclusive and environmentally sustainable—but few economies are on such a trajectory"
...
```{r}
ca_fviz <- select(ca, -starts_with("gdp"), -country_code, -starts_with("p_"))


ca_fviz <- select(ca, gci_overall, annual_growth, gini_index, ggi_index, renewable_cons_prop, nat_cap_score)
...
```{r}
pillars_clust <- kmeansselect(ca, gci_overall, annual_growth, gini_index, ggi_index, renewable_cons_prop, nat_cap_score), centers = 3)
```
